...
- 自动验证：`npm run typecheck`、`npm test`、`npm run build` 通过；当前为 17 个测试文件、58 项测试，覆盖单一主二维码、可靠初始入口、可撤销路径凭据、跨来源授权接力、每个来源独立会话、恶意候选过滤、局域网与 Tailscale 双向切换、最新任务日志、设备命名和身份复用。
