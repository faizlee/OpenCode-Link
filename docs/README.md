...
- 自动验证：`npm run typecheck`、`npm test`、`npm run build` 通过；当前为 16 个测试文件、47 项测试，包含二维码唯一授权、未配对拒绝、手机型号默认命名、泛化名称升级、用户改名保护、重复扫码和跨入口身份复用，以及 Tailscale 地址分类，另有 PowerShell 5.1 身份/语法、隔离 Headless 托盘 IPC，以及便携包清单与隔离冒烟。
