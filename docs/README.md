...
- 实现提交：`3c97bac`（`feat: complete OpenCodex Link mobile delivery`）。
- 自动验证：`npm run typecheck`、`npm test`、`npm run build` 通过；当前为 12 个测试文件、30 项测试。
- 本机宿主验证：设置页、任务列表、历史同步、移动端布局、桌面入口说明、可信设备重启恢复与解除、固定名称 80→8787 转发通过。
