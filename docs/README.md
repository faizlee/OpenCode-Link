...
- 自动验证：`npm run typecheck`、`npm test`、`npm run build` 通过；当前为 16 个测试文件、49 项测试，新增覆盖同一任务多份 JSONL 时选择最新有效日志，以及服务缓存旧日志后自动换源；其余覆盖二维码唯一授权、未配对拒绝、设备命名、重复扫码、跨入口身份复用和 Tailscale 地址分类。
