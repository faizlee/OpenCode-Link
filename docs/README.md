...
- `DESKTOP-TRAY-WEB-CONSOLE-001` 仍保持 `contract_ready`：托盘 Owner、五区 Redline 管理台和便携包已有实现候选；2026-08-29 新确认的“一次扫码绑定设备、连接路径内部无感切换”已写入合同，当前源码的三个二维码选项是明确待实施增量，不能作为正式设计验收。
...
- 下一步需由用户另行授权实施单二维码与内部连接选择，再完成本机和真实手机验收；在此之前不把状态提升为 `delivery_integrated`，也不做 force、PR 或 release。
