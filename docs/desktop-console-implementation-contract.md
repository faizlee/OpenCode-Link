...
> 状态：`contract_ready` / 双快捷入口为 `implementation_pending`；旧自动接力源码不是新合同的完成证据，也不是 `delivery_integrated`
> 基线：分支 `codex/desktop-tray-redline`，工作区 `E:/work/project/codexPWA`
> 实施主控：Cursor CLI
> 用户授权：2026-08-27 曾授权旧方案实施；2026-08-29 确认双快捷入口产品决定，但尚未授权修改运行代码
...
以下先区分当前源码与新合同，避免把旧候选误报为新方案已经完成。

2026-08-29 最新确认决定：管理台仍只显示一个设备授权二维码；扫码成功后提供 `OpenCodex Link · 局域网` 与 `OpenCodex Link · 远程` 两个手机桌面快捷入口。局域网入口使用物理局域网或固定名称，远程入口使用同一 Tailnet 内的 Tailscale 地址。两个浏览器来源可以拥有独立 HttpOnly 会话，但必须映射到同一条受信任设备记录，不得再次扫码、要求密码或新增设备。用户只按场景选择清楚命名的图标，不负责查找技术链接。

当前源码仍是旧“自动探测并跨来源接力”候选；它包含设备级可撤销路径凭据和自动跳转逻辑，但没有完成上述两个快捷入口的设置界面和真实手机流程。该源码只能作为可复用技术基础，不能作为新合同验收证据。新决定获得明确实施授权前不得修改运行代码。
...
- 双入口授权只能由已认证设备派生，使用设备主令牌哈希签名，不暴露主会话令牌；局域网和远程来源分别验证后签发独立 HttpOnly 会话。目标只允许物理私网、`opencodexlink.local`、Tailscale CGNAT 与 `.ts.net`，不得把凭据带到任意公网来源。
- 配对成功页负责依次建立两个入口。若 Tailscale 当时不可达，应保留同一次绑定并给出“连接 Tailscale 后完成远程图标”的明确操作，不得要求第二张二维码或创建第二台设备。不得继续把后台自动换地址作为主要用户体验。
...
- 主操作“添加手机”；复制原始访问地址只保留在技术详情。
- 二维码只在用户点击后签发，带倒计时和刷新；添加手机主路径只显示一个授权二维码。扫码成功页提供“添加局域网图标”和“添加远程图标”，并用普通语言说明适用场景。用户不得被要求复制、记忆或手工打开技术地址。
...
- 固定名称、当前物理局域网地址、Tailscale 地址、两个快捷入口的就绪状态、80 转发/8787 回退状态；技术值只用于状态和诊断。
- 网络技术详情默认折叠；状态读取不得签发配对票据。
- mDNS 只广播物理局域网地址；Tailscale 用于远程快捷入口和技术状态，不提供独立授权二维码。
...
- Tailscale CGNAT 地址只从 Tailscale 网卡生成远程入口，不进入物理 LAN/mDNS 列表；添加手机界面只显示一个授权二维码，局域网与远程入口都映射同一设备记录。
...
6. 五区可导航；二维码按需且只显示一个绑定入口；扫码成功后可分别添加“局域网”和“远程”两个桌面快捷图标，两个入口不暴露原始地址并对应同一设备记录；设备改名/单删/批量；连接状态不发票据。
...
新决定未获明确实施授权前，停止在文档与合同同步，不修改代码、配置、运行服务或便携包。实施后若遇到无法证明的进程身份、需要破坏现有设备身份、必须引入新大型运行时、产品语义冲突或无法保留现有数据，立即停止并报告。
