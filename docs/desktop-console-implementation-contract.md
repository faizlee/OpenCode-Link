...
- `.env` 仍属于安装目录；覆盖同一目录时保留已有密码。换目录启动不得覆盖设备登记表。
...
`/setup/*` 必须始终进入管理台；手机 `/`、`/thread/*`、消息、附件和登录职责不变。

### 概览

- 服务状态、Codex 可用性、服务/托盘版本、设备数、固定地址状态和异常。
- 主操作“添加手机”，次操作“复制访问地址”。
- 二维码只在用户点击后签发，带倒计时和刷新；技术地址默认折叠。
...
- 固定名称、当前物理局域网地址、推荐入口、80 转发/8787 回退状态。
...
6. 五区可导航；二维码按需；设备改名/单删/批量；连接状态不发票据。
