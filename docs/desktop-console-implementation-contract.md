...
2026-08-29 新确认并获授权实施的产品决定覆盖旧的多入口选择：用户绑定的是手机与电脑，添加手机只显示一个由服务端选定可靠初始入口的二维码；局域网、固定名称、Tailscale 与后续连接方式由软件内部探测和切换。当前源码、测试与便携包已经移除三个用户选项，并支持多个内部迁移票据同时有效且各自只使用一次；仍需真实手机和目标机验收，不能据此写成 `delivery_integrated`。
...
- 主操作“添加手机”，次操作“复制访问地址”。
- 二维码只在用户点击后签发，带倒计时和刷新；添加手机主路径只显示一个二维码，软件自动选择当时可达的入口，局域网、固定名称与 Tailscale 选择全部退出用户操作面并收进技术详情。
