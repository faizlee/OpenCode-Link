...
- 主操作“添加手机”，次操作“复制访问地址”。
- 二维码只在用户点击后签发，带倒计时和刷新；局域网与 Tailscale 入口可明确切换，技术地址默认折叠。
...
- 固定名称、当前物理局域网地址、独立 Tailscale 入口、推荐入口、80 转发/8787 回退状态。
- 网络技术详情默认折叠；状态读取不得签发配对票据。
- mDNS 只广播物理局域网地址；Tailscale 只用于显式二维码、连接状态和已授权设备的同身份迁移。
...
- Tailscale CGNAT 地址只从 Tailscale 网卡进入独立直连列表，不进入物理 LAN/mDNS 列表；跨入口迁移不新增设备记录。
- 日志读取覆盖“启动时已有新旧两份同 ID 文件”和“运行中新增更晚文件”两种路径；两者都必须返回新文件内容。
...
6. 五区可导航；二维码按需并可选择局域网/Tailscale；设备改名/单删/批量；连接状态不发票据。
