...
- 点击任务回答中引用的本地图片、PDF或文本文件时，在当前任务内安全预览；不支持格式保留下载入口
- 命令、文件修改、推理和工具过程留在电脑端，不在手机端展示
- 手机息屏或断线后保留当前任务内容并自动重连；重连不刷新全部任务列表，当前任务只有检测到变化时才读取完整内容
...
任务回答中的本地文件链接会由服务端核对“当前任务明确引用、文件真实存在、位于任务工作目录或附件目录”后换成短时预览链接。手机上使用全屏覆盖层，电脑上使用居中弹窗：图片直接查看，PDF由浏览器内嵌阅读，文本、Markdown、日志和代码只读显示；Office、压缩包等不可靠格式只显示文件信息并提供下载。预览和下载都要求已配对设备，HTML、SVG和未知主动内容不会被内联执行。文件已按规则清理、移动或链接过期时会明确报错，重新打开任务可刷新仍然有效的链接。

## 当前验证边界

- 当前源码必须通过 `npm run typecheck`、`npm test` 和 `npm run build`；当前为21个测试文件、67项测试，覆盖任务变化版本、会话内任务缓存、文件预览、任务引用、允许根目录、认证、短时令牌、主动内容阻断、下载与文件删除。
- 本机宿主验证覆盖设置页、任务列表、历史同步、移动端布局、任务内图片/文档预览、可信设备重启恢复与解除、固定名称端口转发、Tailscale 地址发现和 Windows 便携包。
...
Desktop 占用的任务通过本机队列接收手机消息。PWA 在前台每 5 秒轻量检查当前任务的版本，只有版本变化时才读取完整任务；息屏和后台期间暂停检查，重新可见时立即核验一次。任务事件仍会实时更新当前页面，长任务经过上下文压缩后也会跟随最新日志；页面标题下显示最近同步时间。回答可能比电脑界面晚几秒出现，也不显示尚未写入日志的流式片段。
