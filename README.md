...
二维码只负责第一次新增设备。扫码成功后，手机会登记为长期受信任设备；以后在同一个局域网里直接打开页面或手机桌面入口即可，会话会在每次使用时自动续期，不需要定期重新扫码。只有主动解除该设备、清理手机浏览器数据，或改用另一个访问名称时才重新配对。通过固定名称访问时，电脑局域网 IP 变化不需要重新配对。
...
- 短时二维码只用于新增设备；扫码后的受信任设备没有固定30天失效规则，并在正常使用时持续续期。
