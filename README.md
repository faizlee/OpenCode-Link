...
在概览中点击「添加手机」后扫描唯一的二维码。二维码包含软件当时自动选择的可达地址和一个五分钟有效的临时凭证，不包含 Codex/ChatGPT 登录信息。扫码是唯一授权入口，不再提供手工密码登录。
...
第一次扫码使用软件当时判断最可靠的可达地址。进入后页面会在后台探测固定名称和 Tailscale 等候选路径：可达时自动切换，不可达时继续使用原路径，不要求手机用户判断或配置。电脑端会自动完成局域网名称广播和内部端口转发。如果电脑的默认 HTTP 端口已经被其他程序占用，固定入口会自动退回 `:8787`，主服务仍可正常使用。
