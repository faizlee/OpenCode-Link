...
在概览中点击「添加手机」后扫描唯一的二维码。二维码包含软件自动选择的可达地址和一个五分钟有效的临时凭证，不包含 Codex/ChatGPT 登录信息。电脑检测到 Tailscale 入口时优先把它放进这个唯一二维码；未检测到时自动退回局域网或固定名称。用户不需要查看、复制或手工打开任何技术地址。扫码是唯一授权入口，不再提供手工密码登录。
...
第一次扫码使用软件当时判断最可靠的可达地址；电脑存在 Tailscale 时优先使用这个可跨局域网工作的入口，没有时再使用局域网或固定名称。进入后页面会在后台探测其他候选路径，不要求手机用户寻找地址或选择网络。电脑端会自动完成局域网名称广播和内部端口转发。如果电脑的默认 HTTP 端口已经被其他程序占用，固定入口会自动退回 `:8787`，主服务仍可正常使用。

二维码只负责新增尚未授权的设备。扫码成功后，手机会登记为长期受信任设备；以后直接打开同一入口或手机桌面图标即可，会话会在每次使用时自动续期，不需要密码或定期重新扫码。同一浏览器重复扫码只刷新原设备，不会增加重复记录；首次配对后，页面会保存一个可由电脑端随设备解除而失效的路径凭据，并为每个局域网、固定名称或 Tailscale 地址换取独立的 HttpOnly 会话。旧地址失联时页面持续探测已知候选并携带同一设备身份切换，不依赖旧地址仍然在线。只有主动解除该设备、清理手机浏览器数据，或原设备身份确实丢失后，重新配对才会新增记录。
...
保持 Windows 上的 Codex Desktop 和本服务运行，电脑不要休眠。在电脑管理台点击“添加手机”并扫描唯一二维码；软件会把已检测到的 Tailscale 入口放在首选，没有 Tailscale 时自动使用同一 Wi-Fi。用户不需要查看或输入地址。
