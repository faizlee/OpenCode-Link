...
在概览中点击「添加手机」后扫描唯一的二维码。二维码先使用手机当前最容易扫描的局域网或固定名称入口，并包含一个五分钟有效的临时凭证，不包含 Codex/ChatGPT 登录信息。配对后软件会在后台探测 Tailscale 等其他路径并自动迁移；用户不需要查看、复制或手工打开任何技术地址。扫码是唯一授权入口，不再提供手工密码登录。

用户绑定的是“这台手机与这台电脑”，不是某个 IP、域名或网络入口。局域网 IP、固定名称、Tailscale 以及以后增加的其他连接方式都由软件在内部探测、选择和切换；用户不需要选择网络方式，也不应因为网络变化而重新扫码、重新登录或看到重复设备。
...
第一次扫码优先保证手机在当前局域网能够直接打开。进入后页面通过受限的后台探测页检查固定名称和 Tailscale 等候选路径：可达时携带同一设备身份自动迁移，不可达时继续留在当前入口，不要求手机用户寻找地址或选择网络。电脑端会自动完成局域网名称广播和内部端口转发。如果电脑的默认 HTTP 端口已经被其他程序占用，固定入口会自动退回 `:8787`，主服务仍可正常使用。

二维码只负责新增尚未授权的设备。扫码成功后，手机会登记为长期受信任设备；以后直接打开手机桌面图标或任一已经自动登记过的局域网、固定名称、Tailscale 入口即可，会话会在每次使用时自动续期，不需要密码或定期重新扫码。同一浏览器重复扫码只刷新原设备，不会增加重复记录；首次配对后，页面会保存一个可由电脑端随设备解除而失效的路径凭据，逐个探测当前可达入口，并通过短暂的页面接力为每个来源换取独立的 HttpOnly 会话后返回原页面。旧地址失联时页面持续探测已知候选并携带同一设备身份切换，不依赖用户再次携带二维码参数。只有主动解除该设备、清理手机浏览器数据，或原设备身份确实丢失后，重新配对才会新增记录。

电脑运行 Tailscale 时，软件会把它加入内部连接候选，连接页可在技术详情中显示对应状态，但添加手机仍然只有一个二维码。手机加入同一个 Tailnet 且该路径可达时，软件自动沿用原设备身份切换过去。Tailscale 不参加 `.local` 的物理局域网广播，也不等于把服务公开到互联网。
...
`.local` 广播仍只选择真实的 Wi-Fi 或有线网卡地址，会排除 Tailscale、蒲公英、WSL、Hyper-V 等虚拟网卡；Tailscale 只作为内部连接候选和技术状态。停止时双击 `Stop OpenCodex Link.cmd`。
...
保持 Windows 上的 Codex Desktop 和本服务运行，电脑不要休眠。手机连接同一个 Wi-Fi，在电脑管理台点击“添加手机”并扫描唯一二维码；后续 Tailscale 等连接路径由软件在后台处理，用户不需要查看或输入地址。
...
- Tailscale 只允许同一 Tailnet 内能够到达该地址的设备连接；它由软件作为内部候选自动使用，从其他路径切换时复用原设备身份，不要求用户选择或重新扫码。
