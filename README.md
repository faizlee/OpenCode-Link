...
- 手机断线后自动重连
- 同一任务因续接或上下文压缩产生多份本地日志时，自动选择最新有效日志并在运行中重新发现新日志，避免手机内容停在旧文件
...
- 不把电脑浏览器验证冒充手机验收。真实手机上的 `.local` 解析、Tailscale 身份迁移、桌面图标安装和附件发送仍需在目标手机与网络环境中复验；不可用时继续使用扫码得到的局域网入口。
...
在概览中点击「添加手机」后再扫描二维码。二维码只包含可选的局域网或 Tailscale 地址和一个五分钟有效的临时凭证，不包含 Codex/ChatGPT 登录信息。扫码是唯一授权入口，不再提供手工密码登录。
...
二维码只负责新增尚未授权的设备。扫码成功后，手机会登记为长期受信任设备；以后直接打开同一入口或手机桌面图标即可，会话会在每次使用时自动续期，不需要密码或定期重新扫码。同一浏览器重复扫码只刷新原设备，不会增加重复记录；首次配对后从 IP 入口自动切换到固定名称或 Tailscale 入口时，也通过短时迁移票据沿用同一个设备身份。只有主动解除该设备、清理手机浏览器数据，或原设备身份确实丢失后，重新配对才会新增记录。

电脑运行 Tailscale 时，配对面板会增加一个明确的「Tailscale」二维码，连接页也会显示对应地址。手机加入同一个 Tailnet 后可直接使用该地址；已在局域网入口配对的手机若能访问这个 Tailscale 地址，页面会自动把同一设备身份迁移过去。Tailscale 只作为显式直连入口，不参加 `.local` 的物理局域网广播，也不等于把服务公开到互联网。
...
`.local` 广播仍只选择真实的 Wi-Fi 或有线网卡地址，会排除 Tailscale、蒲公英、WSL、Hyper-V 等虚拟网卡；Tailscale 由独立入口显示和配对。停止时双击 `Stop OpenCodex Link.cmd`。
...
- Tailscale 入口只允许同一 Tailnet 内能够到达该地址的设备连接；从已授权入口迁移时复用原设备身份。
